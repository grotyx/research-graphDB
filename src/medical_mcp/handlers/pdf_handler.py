    ...
    async def store_analyzed_paper(
        ...
        try:
            ...
            graph_spine_meta = GraphSpineMetadata(
                ...
                surgical_approach=[],
                pico_population="",
                pico_intervention=interventions[0] if interventions else "",
                pico_comparison=interventions[1] if len(interventions) > 1 else "",
                pico_outcome=", ".join([o.get("name", "") for o in formatted_outcomes if o.get("name")]),
                main_conclusion=summary[:500] if summary else "",
                summary=summary or "",
