...
class PDFHandler:
    ...
    async def add_pdf(
        ...
        try:
            ...
            if use_v7 and hasattr(self.server, 'V7_AVAILABLE') and self.server.V7_AVAILABLE and self.server.v7_processor is not None:
    ...
    async def add_pdf_v7(
        ...
        Returns:
            처리 결과 딕셔너리
        """
        if not hasattr(self.server, 'V7_AVAILABLE') or not self.server.V7_AVAILABLE or self.server.v7_processor is None:
            return {
                "success": False,
        ...
        try:
            ...
            doc_type_enum = None
            if document_type and hasattr(self.server, 'V7DocumentType') and self.server.V7DocumentType:
                try:
                    doc_type_enum = self.server.V7DocumentType(document_type)
                except ValueError:
            ...
            result = await self.server.v7_processor.process_pdf(
                str(path),
                document_type=doc_type_enum
            )

            # Generate document ID
            doc_id = result.paper_id or generate_document_id(
            ...
            if hasattr(self.server, 'GRAPH_AVAILABLE') and self.server.GRAPH_AVAILABLE and self.server.relationship_builder:
                try:
                    ...
                    graph_spine_meta = GraphSpineMetadata(
                        ...
                        interventions=[e.name for e in entities.interventions] if entities and entities.interventions else [],
                        pathologies=[e.name for e in entities.pathologies] if entities and entities.pathologies else [],
                        anatomy_levels=[e.name for e in entities.anatomy_levels] if entities and entities.anatomy_levels else [],
                    ...
                    build_result = await self.server.relationship_builder.build_from_paper(
                        paper_id=doc_id,
                    ...
                    if self.server.neo4j_client and hasattr(self.server, 'ChunkNode') and self.server.ChunkNode and result.chunks:
                        from graph.schema import ChunkNode

                        chunk_nodes = []
                        embeddings = self._generate_embeddings_v7(result.chunks)

                        for i, (chunk, embedding) in enumerate(zip(result.chunks, embeddings)):
                            # Get chunk content - handle both .content and .text attributes
                            chunk_content = getattr(chunk, 'content', None) or getattr(chunk, 'text', '')
                            chunk_section = getattr(chunk, 'section', 'body')
                            ...
                            chunk_node = ChunkNode(
                                ...
                                embedding=embedding,
                                tier="tier1" if chunk_section in ["results", "conclusion"] else "tier2",
                        ...
                        if chunk_nodes:
                            batch_result = await self.server.neo4j_client.create_chunks_batch(doc_id, chunk_nodes)
            ...
            if hasattr(self.server, 'citation_processor') and self.server.citation_processor and result.chunks:
                try:
                    ...
                    citation_result = await self.server.citation_processor.process_from_chunks(
            ...
            return {
                ...
                "pipeline_version": getattr(result, 'processing_version', 'v7.0'),
                "document_type": result.document_type.value if hasattr(result, 'document_type') and result.document_type else "unknown",
                "summary": {
                    "word_count": result.summary.word_count if result.summary else 0,
                    "quality": result.summary.quality.value if result.summary and hasattr(result.summary, 'quality') and result.summary.quality else "unknown",
                ...
                "chunks": {
                    "total": getattr(result, 'chunk_count', len(result.chunks) if result.chunks else 0),
                    "sections": list(set(getattr(c, 'section', 'unknown') for c in result.chunks)) if result.chunks else [],
                },
                "entities": {
                    "has_medical_content": getattr(result, 'has_medical_content', False),
                ...
                "neo4j_graph": {
                    "enabled": hasattr(self.server, 'GRAPH_AVAILABLE') and self.server.GRAPH_AVAILABLE,
    ...
    async def analyze_text(
        ...
        # Legacy path - delegate to server implementation
        logger.info("Using legacy text analysis pipeline")
        return await self.server.analyze_text(text, title, pmid, metadata, use_v7=False)
    ...
    def _generate_embeddings_v7(self, chunks: list) -> list:
        ...
        try:
            ...
            texts = [getattr(chunk, 'content', None) or getattr(chunk, 'text', '') for chunk in chunks]
    ...
    async def _analyze_text_v7(
        ...
        return await self.server._analyze_text_v7(text, title, pmid, metadata)
