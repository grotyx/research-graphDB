...
# v7.0 Simplified Processing Pipeline (2024-12)
# Universal document processing with type detection, summary generation, section-based chunking
try:
    from builder.unified_processor_v7 import (
        UnifiedProcessorV7,
        ProcessingResultV7,
    )
    from builder.document_type_detector import DocumentType as V7DocumentType
    from builder.summary_generator import ContentSummary
    from builder.section_chunker import Chunk as V7Chunk
    from builder.entity_extractor import ExtractedEntities
    V7_AVAILABLE = True
    print(f"[medical_kag_server] UnifiedProcessorV7 imported successfully", file=sys.stderr)
except ImportError as e:
    V7_AVAILABLE = False
    UnifiedProcessorV7 = None
    ProcessingResultV7 = None
    V7DocumentType = None
    ContentSummary = None
    V7Chunk = None
    ExtractedEntities = None
    print(f"[medical_kag_server] UnifiedProcessorV7 import failed: {e}", file=sys.stderr)
...
class MedicalKAGServer:
    ...
    def _init_components(self):
        ...
        self.v7_processor = None      # v7.0 Simplified Processing Pipeline
        ...
        if self.enable_llm and LLM_AVAILABLE:
            ...
            if api_key:
                try:
                    ...
                    # Initialize v7.0 Processor (simplified pipeline)
                    if V7_AVAILABLE and UnifiedProcessorV7 is not None:
                        try:
                            self.v7_processor = UnifiedProcessorV7(
                                llm_client=self.llm_client,
                                auto_detect_type=True,
                                extract_entities=True,
                                target_chunks=20
                            )
                            logger.info("UnifiedProcessorV7 initialized")
                        except Exception as e:
                            logger.warning(f"UnifiedProcessorV7 initialization failed: {e}")
                except Exception as e:
    ...
    async def add_pdf(
        ...
        try:
            # v7.5: v7.0 Simplified Pipeline 우선 사용 (권장)
            if use_v7 and V7_AVAILABLE and self.v7_processor is not None:
                logger.info("Using v7.5 Simplified Pipeline (primary)")
                return await self.add_pdf_v7(file_path, metadata)

            # Fallback 1: Legacy Unified PDF Processor
            if use_vision and self.vision_processor is not None:
                logger.info("Falling back to Legacy Unified PDF processor")
                return await self._process_with_vision(path, metadata)

            # Fallback 2: 기존 멀티스텝 파이프라인
    ...
    async def add_pdf_v7(
        ...
        """PDF 논문 추가 (v7.0 Simplified Pipeline).

        v7.0 파이프라인:
        - 문서 유형 자동 감지 (또는 명시적 지정)
        - 700+ word 요약 생성
        - 섹션 기반 청킹
        - 조건부 엔티티 추출 (의학 콘텐츠만)
        - 새로운 60/40 랭킹 공식 적용
        ...
        Args:
            file_path: PDF 파일 경로
            metadata: 추가 메타데이터
            document_type: 문서 유형 (None=자동감지, "journal-article", "book", etc.)

        Returns:
            처리 결과 딕셔너리
        """
        if not V7_AVAILABLE or self.v7_processor is None:
            return {
                "success": False,
                "error": "v7.0 processor not available. Use add_pdf() instead."
            }
        ...
        try:
            ...
            doc_type_enum = None
            if document_type and V7DocumentType:
                try:
                    doc_type_enum = V7DocumentType(document_type)
                except ValueError:
                    logger.warning(f"Invalid document_type: {document_type}, using auto-detect")

            # Process with v7.0 pipeline
            logger.info(f"Processing with v7.0 pipeline: {path.name}")
            result: ProcessingResultV7 = await self.v7_processor.process_pdf(
                str(path),
            ...
            title = result.metadata.get("title", path.stem)
            doi = result.metadata.get("doi")
            pmid = result.metadata.get("pmid")

            if pmid:
                existing_paper_id = await self._check_existing_paper_by_pmid(pmid)
            elif doi:
                existing_paper_id = await self._check_existing_paper_by_doi(doi)

            if not existing_paper_id and title:
                existing_paper_id = await self._check_existing_paper_by_title(title)

            # Generate document ID
            if existing_paper_id:
                doc_id = existing_paper_id
                logger.info(f"[v7.14.3] Found existing paper: {doc_id}, will update")
            else:
                doc_id = result.paper_id or self._generate_document_id(
                    {"title": title},
                    path.stem
                )

            # Build Neo4j graph with v7.0 fields
            neo4j_nodes = 0
            neo4j_relations = 0
            neo4j_chunk_count = 0
            neo4j_warnings = []
            ...
            if GRAPH_AVAILABLE and self.relationship_builder:
                try:
                    # Extract entities for spine metadata (if medical content)
                    entities = result.entities

                    # Build GraphSpineMetadata with v7.0 fields
                    graph_spine_meta = GraphSpineMetadata(
                        ...
                        # Medical entities (if available)
                        interventions=entities.interventions if entities else [],
                        pathologies=entities.pathologies if entities else [],
                        anatomy_levels=entities.anatomy_levels if entities else [],
                        outcomes=[],  # v7.0 doesn't extract detailed outcomes
                        main_conclusion=result.summary.main_conclusion if result.summary else "",
                    )

                    # Create compatible ExtractedMetadata
                    from dataclasses import dataclass, field as df

                    @dataclass
                    class V7ExtractedMeta:
                        title: str = ""
                    ...
                    extracted_meta = V7ExtractedMeta(
                        title=result.metadata.get("title", ""),
                        authors=result.metadata.get("authors", []),
                        year=result.metadata.get("year", 0),
                        journal=result.metadata.get("source", ""),
                        doi=result.metadata.get("doi", ""),
                        evidence_level=result.metadata.get("evidence_level", "5"),
                        abstract=result.summary.text[:500] if result.summary else "",
                        spine=graph_spine_meta,
                    )

                    # Build graph relations (v7.5: 멀티유저 지원)
                    build_result = await self.relationship_builder.build_from_paper(
                        paper_id=doc_id,
                        metadata=extracted_meta,
                        spine_metadata=graph_spine_meta,
                        chunks=[],  # v7.0 uses different chunk format
                    ...
                    logger.info(f"Neo4j Graph (v7.0): {neo4j_nodes} nodes, {neo4j_relations} relations")

                    # Store v7.0 chunks in Neo4j
                    if self.neo4j_client and ChunkNode and result.chunks:
                        chunk_nodes = []
                        embeddings = self._generate_embeddings_v7(result.chunks)

                        for i, (chunk, embedding) in enumerate(zip(result.chunks, embeddings)):
                            chunk_node = ChunkNode(
                                chunk_id=f"{doc_id}_v7_chunk_{i}",
                                paper_id=doc_id,
                                content=chunk.content,
                                embedding=embedding,
                                tier="tier1" if chunk.section in ["results", "conclusion"] else "tier2",
                        ...
                        if chunk_nodes:
                            ...
                            logger.info(f"Neo4j Chunks (v7.0): {neo4j_chunk_count} chunks stored")

                except Exception as e:
                    logger.warning(f"Neo4j integration (v7.0) failed: {e}")
                    neo4j_warnings.append(str(e))

            # === Important Citation Processing (v7.5) ===
            citation_result = None
            if self.citation_processor and result.chunks:
                try:
                    logger.info("Processing important citations (v7.5)...")
                    ...
                    # 주요 발견사항 추출
                    main_findings = []
                    if result.summary and result.summary.main_conclusion:
                        main_findings.append(result.summary.main_conclusion)
                    ...
                    citation_result = await self.citation_processor.process_from_chunks(
                        ...
                        paper_title=result.metadata.get("title", "")
                    )

                    if citation_result:
                        logger.info(
                            f"Citation processing (v7.5): "
                            f"{citation_result.important_citations_count} important citations, "
                            f"{citation_result.papers_created} papers, "
                            f"{citation_result.relationships_created} CITES relations"
                        )

                except Exception as e:
                    logger.warning(f"Important Citation processing (v7.5) failed: {e}")

            return {
                "success": True,
                "document_id": doc_id,
                "processing_method": "v7.5_simplified",
                "pipeline_version": result.processing_version,
                "document_type": result.document_type.value if result.document_type else "unknown",
                "summary": {
                    "word_count": result.summary.word_count if result.summary else 0,
                    "quality": result.summary.quality.value if result.summary and result.summary.quality else "unknown",
                    "preview": result.summary.text[:200] + "..." if result.summary and len(result.summary.text) > 200 else (result.summary.text if result.summary else ""),
                },
                "chunks": {
                    "total": result.chunk_count,
                    "sections": list(set(c.section for c in result.chunks)) if result.chunks else [],
                },
                "entities": {
                    "has_medical_content": result.has_medical_content,
                    "interventions": result.entities.interventions if result.entities else [],
                    "pathologies": result.entities.pathologies if result.entities else [],
                },
                "neo4j_graph": {
                    "enabled": GRAPH_AVAILABLE,
                    "nodes_created": neo4j_nodes,
                    "relationships_created": neo4j_relations,
                    "chunks_stored": neo4j_chunk_count,
                ...
                } if citation_result else None,
                "processing_time": result.processing_time,
                "warnings": result.warnings,
            }

        except Exception as e:
            logger.exception(f"Error in v7.5 processing: {e}")
            return {"success": False, "error": str(e)}

    def _generate_embeddings_v7(self, chunks: list) -> list:
        """v7.0 청크에 대한 임베딩 생성.

        Args:
            chunks: v7.0 Chunk 목록
    ...
    async def analyze_text(
        ...
        # v7.5: v7.0 Simplified Pipeline 우선 사용 (권장)
        if use_v7 and V7_AVAILABLE and self.v7_processor is not None:
            logger.info("Using v7.5 Simplified Pipeline for text analysis")
            return await self._analyze_text_v7(text, title, pmid, metadata)

        # Legacy path continues below...
        logger.info("Using legacy text analysis pipeline")
    ...
    async def _analyze_text_v7(
        ...
        """v7.5 Simplified Pipeline을 사용한 텍스트 분석.

        UnifiedProcessorV7.process()를 사용하여 텍스트를 분석합니다.
        - 문서 유형 자동 감지
        - 700+ word 통합 요약
        - 섹션 기반 청킹
        - 조건부 엔티티 추출
        ...
        # 2. Paper ID 생성 (v7.14.3: 중복 방지를 위해 DOI/제목 기반 해시 사용)
        existing_paper_id = None
        doi_from_meta = metadata.get("doi") if metadata else None
        ...
        if pmid:
            ...
            # DOI로 기존 Paper 확인
            existing_paper_id = await self._check_existing_paper_by_doi(doi_from_meta)
            if existing_paper_id:
                paper_id = existing_paper_id
            else:
                safe_doi = doi_from_meta.replace("/", "_").replace(":", "_")
                paper_id = f"doi_{safe_doi}"
        else:
            # 제목 기반 해시로 일관된 ID 생성 (중복 방지)
            title_hash = hashlib.md5(title.lower().strip().encode()).hexdigest()[:12]
            paper_id = f"text_{title_hash}"
            # 제목으로 기존 Paper 확인
            existing_paper_id = await self._check_existing_paper_by_title(title)

        if existing_paper_id:
            paper_id = existing_paper_id
            logger.info(f"[v7.14.3] Found existing paper: {paper_id}, will update instead of create")

        logger.info(f"[v7.5] Analyzing text: {title[:50]}... (paper_id={paper_id}, {len(text)} chars)")

        # 3. v7.0 프로세서로 분석
        try:
            result = await self.v7_processor.process(
                text=text,
                filename=title,
                paper_id=paper_id,
            )
        except Exception as e:
            logger.exception(f"[v7.5] Text analysis failed: {e}")
            return {
                "success": False,
                "error": f"v7.5 분석 실패: {str(e)}"
            }

        # 4. PubMed 서지 정보 enrichment
        metadata = metadata or {}
        pubmed_metadata = None
        pubmed_enriched = False
        web_search_result = None
        ...
        if self.pubmed_enricher:
            try:
                logger.info(f"[v7.5] Attempting PubMed enrichment for: {title[:50]}...")
                pubmed_metadata = await self.pubmed_enricher.auto_enrich(
                    title=title,
                ...
                if pubmed_metadata:
                    pubmed_enriched = True
                    logger.info(f"[v7.5] PubMed enrichment successful: PMID={pubmed_metadata.pmid}")
                else:
                    # Web search fallback
                    web_search_result = await self._web_search_bibliographic_info(
                        title=title,
                    ...
                    if web_search_result:
                        logger.info(f"[v7.5] Web search found bibliographic info")

            except Exception as e:
                logger.warning(f"[v7.5] PubMed enrichment failed: {e}")
        ...
        # 6. Neo4j 관계 구축
        neo4j_result = None
        if self.neo4j_client and self.relationship_builder and result.entities:
            try:
                # GraphSpineMetadata already imported at module level

                # v7.0 엔티티에서 SpineMetadata 생성
                entities = result.entities
                graph_spine_meta = GraphSpineMetadata(
                    sub_domain="Unknown",
                    sub_domains=[],
                    ...
                    # v7.5: summary 필드 추가
                    summary=result.summary.text if result.summary else "",
                    processing_version="v7.5",
                )

                # ExtractedMetadata 호환 객체
                from dataclasses import dataclass, field as df
                ...
                meta_compat = ExtractedMetaCompat(
                    ...
                    journal=journal,
                    doi=doi or "",
                    pmid=pmid or (pubmed_metadata.pmid if pubmed_metadata else "") or "",
                    abstract=result.summary.text[:2000] if result.summary else text[:2000],
                    spine=graph_spine_meta,
                )
                ...
                logger.info(f"[v7.5] Neo4j relationships built: {neo4j_result.nodes_created} nodes")

            except Exception as e:
                logger.warning(f"[v7.5] Neo4j relationship building failed: {e}")

        # 7. 청크 저장 (Neo4j Vector)
        chunks_created = 0
        if result.chunks and self.neo4j_client:
            try:
                from core.embedding import OpenAIEmbeddingGenerator

                embedding_gen = OpenAIEmbeddingGenerator()
                chunk_texts = [c.text for c in result.chunks if hasattr(c, 'text')]
                ...
                if chunk_texts:
                    ...
                    for i, (chunk, embedding) in enumerate(zip(result.chunks, embeddings)):
                        ...
                        await self.neo4j_client.run_query(
                            ...
                            """,
                            {
                                ...
                                "chunk_id": chunk_id,
                                "content": chunk.text,
                                "tier": 1 if chunk.has_statistics else 2,
                                "section": chunk.section,
                                "embedding": embedding
                            }
                        )
                    ...
                    logger.info(f"[v7.5] Stored {chunks_created} chunks with embeddings")

            except Exception as e:
                logger.warning(f"[v7.5] Chunk storage failed: {e}")

        # 8. Important Citation Processing (v7.6)
        citation_result = None
        important_citations_data = []
        if self.citation_processor and result.chunks:
            try:
                logger.info("[v7.6] Processing important citations...")

                # v7.0 청크를 딕셔너리로 변환
                chunks_dicts = []
                for chunk in result.chunks:
                    chunk_content = getattr(chunk, 'text', '') or getattr(chunk, 'content', '')
                ...
                # 주요 발견사항 추출
                main_findings = []
                if result.summary and hasattr(result.summary, 'main_conclusion') and result.summary.main_conclusion:
                    main_findings.append(result.summary.main_conclusion)

                # Discussion/Results 섹션에서 중요 인용 처리 (PubMed enrichment 포함)
                citation_result = await self.citation_processor.process_from_chunks(
                ...
                if citation_result:
                    logger.info(
                        f"[v7.6] Citation processing: "
                        f"{citation_result.important_citations_count} important citations, "
                        f"{citation_result.papers_created} papers created, "
                        f"{citation_result.relationships_created} CITES relations"
                    )

                    # JSON 저장용 데이터 구성
                    important_citations_data = citation_result.citations_data if hasattr(citation_result, 'citations_data') else []

            except Exception as e:
                logger.warning(f"[v7.6] Citation processing failed: {e}")

        # 9. JSON 저장
        pmid_found = pubmed_metadata.pmid if pubmed_metadata else pmid
        try:
            await self._save_analyze_text_json(
                paper_id=paper_id,
                title=title,
                ...
                extracted_data={
                    "summary": result.summary.text if result.summary else "",
                    "summary_word_count": result.summary.word_count if result.summary else 0,
                    "document_type": result.document_type.value if result.document_type else "unknown",
                    "entities": {
                        "interventions": [e.name for e in result.entities.interventions] if result.entities else [],
                        "pathologies": [e.name for e in result.entities.pathologies] if result.entities else [],
                        "outcomes": [e.name for e in result.entities.outcomes] if result.entities else [],
                    } if result.entities else None,
                    "chunks": [
                        {"content": c.text, "section": c.section}
                        for c in result.chunks
                    ] if result.chunks else [],
                    "important_citations": important_citations_data,
                },
                pubmed_metadata=pubmed_metadata,
                web_search_result=web_search_result,
            )
        except Exception as e:
            logger.warning(f"[v7.6] JSON save failed: {e}")
        ...
        return {
            ...
            "paper_id": paper_id,
            "title": title,
            "processing_method": "v7.6_text_analysis",
            "document_type": result.document_type.value if result.document_type else "unknown",
            "summary": {
                "text": result.summary.text if result.summary else "",
                "word_count": result.summary.word_count if result.summary else 0,
                "sections": result.summary.sections if result.summary and result.summary.sections else None,
            } if result.summary else None,
            "entities": {
                "interventions": [e.name for e in result.entities.interventions] if result.entities and result.entities.interventions else [],
                "pathologies": [e.name for e in result.entities.pathologies] if result.entities and result.entities.pathologies else [],
                "outcomes": [e.name for e in result.entities.outcomes] if result.entities and result.entities.outcomes else [],
                "has_medical_content": result.has_medical_content,
            } if result.entities else {"has_medical_content": False},
            "enrichment": {
                "pubmed_enriched": pubmed_enriched,
                "pmid": pmid_found,
                "mesh_terms": pubmed_metadata.mesh_terms if pubmed_metadata else None,
            ...
            "important_citations": {
                "processed": citation_result is not None,
                "count": citation_result.important_citations_count if citation_result else 0,
                "papers_created": citation_result.papers_created if citation_result else 0,
                "cites_relations": citation_result.relationships_created if citation_result else 0,
            } if citation_result else None,
            "stats": {
...
"""
    ...
    async def _import_doi_to_graph(self, doi_result: "DOIFullText") -> dict:
        ...
        # v7.5 파이프라인으로 분석 및 저장
        if text_to_analyze and self.v7_processor:
            try:
                result = await self.analyze_text_v7(
                ...
                logger.warning(f"V7 pipeline failed, falling back: {e}")

        # Fallback: 기본 메타데이터만 저장
        try:
            paper_data = {
                "paper_id": paper_id,
