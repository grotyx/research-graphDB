...
Multi-hop Reasoning 모듈의 각 컴포넌트를 테스트:
...
@pytest.mark.asyncio
async def test_hop_executor_simple_execution(mock_search_pipeline, mock_llm_client):
    """단순 hop 실행 테스트."""
    # Mock 검색 응답
...
@pytest.mark.asyncio
async def test_hop_executor_with_context(mock_search_pipeline, mock_llm_client):
    """컨텍스트가 있는 hop 실행 테스트."""
    # Mock 검색 응답
    mock_search_response = MagicMock()
    mock_search_response.results = [
...
async def test_hop_executor_confidence_calculation(mock_search_pipeline, mock_llm_client):
    ...
    mock_search_response = MagicMock()
    mock_search_response.results = [
        MagicMock(final_score=0.95, evidence_level="1a"),
        MagicMock(final_score=0.90, evidence_level="1b"),
        MagicMock(final_score=0.85, evidence_level="2a"),
    ]
...
def test_reasoning_chain_summary():
    ...
    chain.add_step(sq1, "TLIF is a fusion technique", [], 0.9, "Based on evidence")
...
@pytest.mark.asyncio
async def test_reasoner_single_hop(mock_search_pipeline, mock_llm_client):
    """단일 hop 추론 테스트."""
    # Mock decomposer
    ...
    mock_search_response = MagicMock()
    mock_search_response.results = [
        MagicMock(paper_id="p1", title="TLIF Overview", publication_year=2023, final_score=0.9, evidence_level="1b")
    ]
    mock_search_response.synthesis = None
    mock_search_pipeline.search.return_value = mock_search_response
...
@pytest.mark.asyncio
async def test_reasoner_two_hops(mock_search_pipeline, mock_llm_client):
    """2-hop 추론 테스트."""
    # Mock decomposer
    mock_llm_client.generate_json.side_effect = [
        # Decomposition
    ...
    # Mock search (called twice)
    mock_search_response = MagicMock()
    mock_search_response.results = [
        MagicMock(paper_id=f"p{i}", title=f"Study {i}", publication_year=2023, final_score=0.9, evidence_level="2a")
        for i in range(3)
    ]
    mock_search_response.synthesis = None
    mock_search_pipeline.search.return_value = mock_search_response
...
@pytest.mark.asyncio
async def test_integration_comparative_query(mock_search_pipeline, mock_llm_client):
    ...
    mock_search_response = MagicMock()
    mock_search_response.results = [
        MagicMock(paper_id=f"p{i}", title=f"Study {i}", publication_year=2023, final_score=0.85, evidence_level="2a")
        for i in range(5)
    ]
    mock_search_response.synthesis = None
    mock_search_pipeline.search.return_value = mock_search_response
    ...
    # Execution plan 확인 (q1, q2는 병렬)
    # reasoner._create_execution_plan 호출 확인은 내부 로직이므로 생략
...
@pytest.mark.asyncio
async def test_hop_result_to_context():
