#!/usr/bin/env python3
"""Import remaining Topic 6 (Tumor/SINS) and Topic 7 (Osteotomy/Deformity) into Neo4j.

Pipeline per paper:
  1. Fetch fulltext via PMC (save to data/fulltext/{pmid}.txt)
  2. Extract entities via LLM (Claude Sonnet via UnifiedPDFProcessor)
  3. Import to Neo4j: Paper node, relationships, embeddings, chunks
  4. Set sub_domain and evidence_level

Usage:
    cd /Users/sangminpark/Documents/rag_research
    PYTHONPATH=./src python3 scripts/import_topic6_7_remaining.py
"""
...
logger = logging.getLogger("import_topic6_7")
...
async def process_single_paper(
    ...
    if isinstance(authors_raw, str):
        ...
        authors = authors_raw[:10] if isinstance(authors_raw, list) else []
...
async def main():
    ...
    # Topic files
    topic_files = [
        ("/tmp/remaining_topic6_tumor.json", "Tumor", "SINS/Tumor"),
        ("/tmp/remaining_topic7_osteotomy.json", "Deformity", "PSO/VCR Osteotomy"),
    ]
