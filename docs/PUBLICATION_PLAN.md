...
### 2.1 전체 구조: 6편 논문, 3 Phase

```
Phase 0 (공통)     Gold Standard Dataset 구축 ─────────────────────┐
                                                                    │
Phase 1 (기반)     Paper 1: System Architecture ◄──────────────────┤
                                                                    │
Phase 2 (확장)     Paper 2: SNOMED Ontology ◄──────────────────────┤
                   Paper 3: Evidence Synthesis (임상) ◄────────────┤
                                                                    │
Phase 3 (응용)     Paper 4: Clinical Decision Support ◄────────────┤
                   Paper 5: Writing Guide (Short Comm) ◄───────────┤
                   Paper 6: SpineBench Dataset ◄───────────────────┘
```

모든 논문이 Phase 0의 Gold Standard Dataset을 공유하며,
Paper 1을 나머지 5편이 인용 → **자기 인용 클러스터** 형성.
...
### Paper 5: EQUATOR Writing Guide (Short Communication)

| 항목 | 내용 |
|------|------|
| **제목(안)** | "AI-Assisted EQUATOR Checklist Compliance Assessment for Medical Manuscripts: A Pilot Evaluation" |
| **타겟 저널** | **1순위**: Research Integrity and Peer Review (IF ~3.5) |
| | **2순위**: BMC Medical Research Methodology |
| **논문 유형** | Short Communication / Technical Note |

#### 실험 설계 (v2)

| 실험 | 내용 | 메트릭 | annotation 필요? |
|------|------|--------|:---:|
| **Exp 1**: 자동 vs 수동 체크리스트 | 출판된 논문 50편의 EQUATOR 항목 준수율: 시스템 평가 vs 연구자 수동 평가 | Cohen's κ (일치도), Sensitivity, Specificity | 최소 (1명) |
| **Exp 2**: Before/After | 전공의/학생 10명의 초고에 시스템 가이드 적용 | 수정 전후 체크리스트 준수율 변화 (paired t-test) | 필요 (IRB) |
| **Exp 3**: LLM-as-Judge 검증 | LLM의 체크리스트 평가와 수동 평가 비교 | LLM Judge 정확도, 재현성 | 불필요 |

> Exp 1+3은 자동으로 가능. Exp 2만 IRB 필요 (Paper 4와 공유).
> 논문 50편 EQUATOR 평가는 LLM-as-Judge로 자동화 후, 수동 평가와 일치도를 보고.

---

### Paper 6: SpineBench Dataset (Resource Paper)

| 항목 | 내용 |
|------|------|
| **제목(안)** | "SpineBench: A Benchmark Dataset for Evaluating Medical Knowledge Graph Retrieval Systems in Spine Surgery" |
| **타겟 저널** | **1순위**: Scientific Data (Nature, IF ~9.8) |
| | **2순위**: Data in Brief |
| | **대안**: arXiv preprint + GitHub 공개 |
| **논문 유형** | Data Descriptor |

#### 공개 데이터셋 구성 (v2 — End-to-End 평가 포함)

```
SpineBench/
├── queries/
│   ├── clinical_questions.json       # 29+ 질문 (5개 sub-domain)
│   └── query_metadata.json           # domain, type, difficulty
├── ontology/
│   ├── spine_snomed_735.csv          # SNOMED-CT 매핑 (735코드)
│   ├── is_a_hierarchy.csv            # parent-child 관계
│   └── normalization_aliases.csv     # 280+ 동의어 매핑
├── baselines/
│   ├── answers/                      # 각 baseline의 생성된 답변
│   │   ├── B1_keyword.json
│   │   ├── B2_vector.json
│   │   ├── B3_llm_direct.json
│   │   └── B4_graphrag.json
│   └── scores/                       # RAGAS 스타일 평가 결과
│       └── comparison.json
├── evaluation/
│   ├── metrics.py                    # 전통 IR + RAGAS 메트릭
│   ├── llm_judge.py                  # LLM-as-Judge 평가 코드
│   └── baseline_scores.json          # 전체 baseline 성능
└── README.md
```

#### 이 논문의 전략적 가치

- 다른 5편 논문에서 **모두 인용** → 자기 인용 6건 확보
- 커뮤니티에서 다른 시스템 벤치마크에 사용 시 **피인용 급증**
- Nature Scientific Data는 IF 9.8로 **가장 높은 IF**
- Phase 0 데이터 + Paper 1-3 결과를 정리하면 되므로 **추가 실험 불필요**
...
```
      ...
      ┌──────┬──────┬──────┬──────┬──────┬──────┬──────┬──────┬──────┬──────┬──────┬──────┐
Ph 0  │██████│██████│      │      │      │      │      │      │      │      │      │      │
      │Gold Standard, DB 확보, Evaluation 코드, Baseline 구현                             │
      ├──────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┤
P1    │      │      │██████│██████│ Rev  │      │      │      │      │      │      │      │
      │      │      │실험+집필│투고  │리비전│      │      │      │      │      │      │      │
      ├──────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┤
...
| 역할 | P1 (System) | P2 (SNOMED) | P3 (Evidence) | P4 (CDS) | P5 (Writing) | P6 (Bench) |
|------|:-----------:|:-----------:|:-------------:|:---------:|:------------:|:----------:|
| 1저자 | **박상민** | **박상민** | 전공의A/박상민 | 전공의A | 전공의B/학생 | **박상민** |
| 공저 | 정보의학 동료 | 정보의학 | 박상민 | 다기관 공저 | 박상민 | 전체 팀 |
| 교신 | **박상민** | **박상민** | **박상민** | **박상민** | **박상민** | **박상민** |

#### 인력 활용 전략

- **전공의 A**: Paper 3 (Evidence Chain) 실험 + Paper 4 (CDS) 실험 주도 → 2편 참여
- **전공의 B / 학생**: Paper 5 (Writing Guide) 실험 → 1편 참여
...
| P5 | Writing Guide | RIPR | ~3.5 | Short Comm | 50편 EQUATOR 자동 평가 + LLM-as-Judge 검증 |
| P6 | SpineBench | Scientific Data | ~9.8 | Data | 공개 데이터셋 + 답변 + 평가 코드 |

**총 6편** | 평균 IF ~5.5 | 1저자/교신 6편 확보

> **v2 핵심 변경**: Paper 1, 3, 5는 LLM-as-Judge로 대부분 자동 평가 가능 → 전문의 의존도 ↓
> Paper 4만 전문의 참여 필수 (임상 저널 특성상 필요)
...
- [x] Neo4j DB 624편 확보 (TR+20, TU+20, BS+20 임포트)
- [x] `evaluation/` 디렉토리 + metrics.py (28 tests passed)
- [x] baselines.py (B1~B4 4종)
- [x] benchmark.py (CLI 실행기)
- [x] Gold Standard 29개 질문
- [x] annotation_sheet.json (517편 후보)
- [x] 60편 entity 추출 + summary + embedding (Haiku API 비용 0)

### 다음 단계: Paper 1 실험 준비

- [ ] `evaluation/answer_generator.py` — 각 baseline 검색 → LLM 답변 생성
- [ ] `evaluation/llm_judge.py` — Faithfulness, Citation Fidelity, Relevancy 평가
- [ ] `evaluation/ragas_metrics.py` — RAGAS 스타일 메트릭 통합 계산
- [ ] 29개 질문 × 4 baselines 파일럿 실행 (5개 질문 먼저)
- [ ] Paper 1 Introduction + Methods 초안
...
| **P5 (Writing)** | EQUATOR 자동 평가 vs 수동 일치도 (Cohen's κ) | **80%** | 1명 수동 평가 대조 |
| **P6 (Benchmark)** | 데이터셋 공개 — 평가 코드 + baseline 결과 포함 | **100%** | 불필요 |

> **전략**: P1, P2, P5, P6은 자동 평가 중심 → 빠르게 진행 가능.
> P3, P4는 전문의 참여 필요 → 타임라인 후반에 배치.
