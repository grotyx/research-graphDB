...
> **Version**: 1.32.0 | **Updated**: 2026-03-17
...
**상태:** ✅ 완료 (v1.31.0) — `search_dao.multi_vector_search()` RRF 기반 chunk+paper 임베딩 병합, `use_multi_vector=True` 옵션
...
**상태:** ✅ 완료 (v1.31.0) — `calculate_weighted_effect_size()`, `calculate_i_squared()`, `generate_forest_plot_data()` 추가 (Cochrane method)
...
**상태:** ✅ 완료 (v1.31.0) — `ClinicalQueryDecomposer`, `ResultAggregator`, `agentic_solve()` 파이프라인 구현
...
**상태:** ✅ 완료 (v1.31.0) — `scripts/neo4j_backup.py` 신규, APOC JSON export + volume tar 이중 백업, restore/list/cleanup 지원
...
| # | 항목 | 효과 | 난이도 |
|---|------|------|--------|
| 9 | ~~2.3 Agentic RAG~~ | ~~다단계 추론~~ | ✅ v1.31.0 |
| 10 | ~~2.2 Evidence Synthesis 강화~~ | ~~메타분석 자동화~~ | ✅ v1.31.0 |
