...
| `build_relations` | 논문 간 SIMILAR_TOPIC 관계 자동 구축 (v7.14.16+) |
...
수술법의 계층 구조와 비교 분석을 수행합니다. v7.14.6에서 IS_A 계층이 확장되었습니다.
