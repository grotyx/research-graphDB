...
> **v1.24.x 참고**: entity_normalizer alias 대폭 확장 (~280 변형 추가, 166 orphan 동기화). `_build_reverse_map` last-write-wins 패턴 주의 — 새 canonical이 기존 alias를 덮어쓸 수 있음.

### 4.7 SNOMED 미매핑 용어 LLM 제안 🤖

SNOMED 코드가 없는 엔티티를 대상으로 `snomed_proposer.py`(LLM)가 적절한 SNOMED 매핑을 제안합니다.
...
with driver.session() as s:
    ...
    for label in ['Intervention', 'Pathology', 'Outcome', 'Anatomy']:
        ...
        ''')
        unmapped = [rec['name'] for rec in r]
        total_r = s.run(f'MATCH (n:{label}) RETURN count(n) AS total')
        total = total_r.single()['total']
        pct = (total - len(unmapped)) * 100 // total if total > 0 else 0
        status = '✅' if pct >= 80 else '⚠️' if pct >= 50 else '❌'
        print(f'{label}: {total - len(unmapped)}/{total} ({pct}%) SNOMED 매핑됨 {status}')
...
**LLM 제안 실행 (수복):**
```bash
# Dry-run: 제안만 출력 (DB 변경 없음)
PYTHONPATH=./src python3 -c "
import asyncio
from ontology.snomed_proposer import SNOMEDProposer
async def main():
    proposer = SNOMEDProposer()
    # 미매핑 용어 자동 수집 + LLM 제안
    suggestions = await proposer.propose_unmapped(dry_run=True)
    for s in suggestions:
        print(f'{s.entity_type}: {s.name} → {s.proposed_code} ({s.confidence})')
asyncio.run(main())
"

# 실제 적용: snomed_proposer 제안 → spine_snomed_mappings.py에 추가 필요
```

**기대 결과:** 미매핑 비율 감소 추세. 제안은 사람이 검토 후 적용

---

## Phase 5: 데이터 품질 (병렬 실행)

> 콘텐츠 품질, 분포 이상, 중복, Summary/엔티티 품질을 점검합니다.
...
Paper에 LLM 생성 summary가 없는 건을 탐지합니다. `backfill_summary.py`로 소급 생성 가능합니다.
...
driver = GraphDatabase.driver('bolt://localhost:7687', auth=('neo4j', os.environ['NEO4J_PASSWORD']))
with driver.session() as s:
    print('=== Paper Summary 누락 탐지 ===')
    r = s.run('''
        MATCH (p:Paper)
        WITH count(p) AS total,
            sum(CASE WHEN p.summary IS NOT NULL AND size(p.summary) > 100 THEN 1 ELSE 0 END) AS has_summary
        RETURN total, has_summary
    ''')
    rec = r.single()
    total, has = rec['total'], rec['has_summary']
    missing = total - has
    pct = has * 100 // total if total > 0 else 0
    status = '✅' if pct >= 90 else '⚠️' if pct >= 70 else '❌'
    print(f'Summary 있음: {has}/{total} ({pct}%) {status}')
    print(f'Summary 누락: {missing}')

    if missing > 0:
        r2 = s.run('''
            MATCH (p:Paper)
            WHERE p.summary IS NULL OR size(p.summary) <= 100
            RETURN p.paper_id AS id, p.title AS title
            ORDER BY p.paper_id
            LIMIT 10
        ''')
        for rec2 in r2:
            title = (rec2['title'] or '')[:60]
            print(f'  - {rec2[\"id\"]}: {title}...')
driver.close()
"
```
...
동일하거나 유사한 이름의 엔티티를 탐지합니다. `consolidate_entities.py`(LLM)로 병합 여부를 판별할 수 있습니다.
...
driver = GraphDatabase.driver('bolt://localhost:7687', auth=('neo4j', os.environ['NEO4J_PASSWORD']))
with driver.session() as s:
    print('=== 중복/유사 엔티티 후보 탐지 ===')
    for label in ['Intervention', 'Pathology', 'Outcome', 'Anatomy']:
        # 대소문자 무시 중복
        r = s.run(f'''
            MATCH (n:{label})
            WITH toLower(trim(n.name)) AS lname, collect(n.name) AS names
            WHERE size(names) > 1
            RETURN lname, names
            ORDER BY lname
        ''')
        ...
        r2 = s.run(f'''
            MATCH (n:{label})
            WITH left(toLower(trim(n.name)), 10) AS prefix, collect(n.name) AS names
            WHERE size(names) > 1 AND size(names) <= 5
            RETURN prefix, names
            ORDER BY prefix
            LIMIT 10
        ''')
        similar = [rec for rec in r2]
        print(f'{label} 유사 이름 후보 (prefix 10자): {len(similar)}')
...
```bash
# Dry-run: 병합 후보만 출력
PYTHONPATH=./src python3 scripts/consolidate_entities.py --dry-run --entity-type Intervention

# 실행 (LLM으로 중복 판별 + 병합)
PYTHONPATH=./src python3 scripts/consolidate_entities.py --entity-type Intervention --batch-size 20
```

**기대 결과:** 대소문자 중복 = 0 ✅, 유사 이름은 사람이 LLM 결과 검토 후 병합
...
| 4.7 SNOMED 미매핑 LLM 제안 🤖 | ✅/⚠️ | 미매핑: I:X, P:X, O:X, A:X |
...
| 5.3 중복 논문 | ✅/❌ | |
| 5.4 Chunk 품질 | ✅/❌ | |
| 5.5 Summary 누락 🤖 | ✅/⚠️ | 누락: X건, 비율: X% |
| 5.6 중복/유사 엔티티 🤖 | ✅/⚠️ | 대소문자 중복: X, 유사: X |
...
Phase 4 (병렬 7개)     └─ 2.6 INVOLVES 완전성   ├─ 5.1 콘텐츠 품질
├─ 4.1 DOI/PMID                                ├─ 5.2 분포 이상
├─ 4.2 SNOMED 커버리지  Phase 6 (병렬 9개)      ├─ 5.3 중복 논문
├─ 4.3 SNOMED 유효성    ├─ 6.1 IS_A 완전성      ├─ 5.4 Chunk 품질
├─ 4.4 SNOMED 중복      ├─ 6.2 IS_A 순환        ├─ 5.5 Summary 누락 🤖
├─ 4.5 SNOMED 소스 동기화├─ 6.3 고아 리프        └─ 5.6 중복/유사 엔티티 🤖
├─ 4.6 Normalizer 커버리지├─ 6.4 SNOMED 커버리지
└─ 4.7 SNOMED LLM 제안 🤖├─ 6.5 parent_code 일치
                        ├─ 6.6 TREATS/AFFECTS
                        ├─ 6.7 AFFECTS 통계
                        ├─ 6.8 SNOMED 중복
                        └─ 6.9 IS_A 라벨 일관성
...
| DV-A-007 | 6.3 | 'Automated Radiograph Analysis' IS_A root node | AI 기반 영상 분석 카테고리의 정당한 root node (5개 하위 노드 보유). IS_A 계층에서 최상위 카테고리로 유지 | 2026-02-28 |
| DV-A-008 | 4.2/6.4 | SNOMED 커버리지 수치 (I:44%, P:57%, O:26%, A:91%) | 매핑 653개 + enrich 완료. 커버리지는 신규 논문 임포트 시 점진적 개선 예정. 정기 모니터링 | 2026-02-28 |
| DV-A-009 | 5.3 | 동일 제목 Paper 1쌍 (pubmed_41523539 vs pubmed_41559891) | 이중 출판 (e-pub ahead of print + 본 출판), DOI 상이. 실제 중복이 아닌 별도 레코드 | 2026-02-28 |
| DV-A-010 | 6.6 | Intervention 'Robot-assisted fusion' AFFECTS 미연결 | 해당 논문이 REPORTS_OUTCOME 관계가 없어 AFFECTS 생성 불가. 향후 논문 추가 시 자연 해소 예정 | 2026-02-28 |
| DV-A-011 | 6.8 | Pathology-Outcome cross-label name overlap 20쌍 | 의학적으로 동일 개념이 Pathology/Outcome 양쪽에 존재 가능 (예: Dysphagia, Radiculopathy, Adjacent Segment Disease 등). 라벨 분리가 의미적으로 타당 | 2026-03-02 |
...
|----|-------|--------|------|------|----------|------|
| DV-NEW-015 | 6.1 | Low | 정보 | IS_A 계층 커버리지 — I:52.4%, P:49.2%, O:83.3%, A:33.7%. SNOMED 매핑 확장 시 점진적 개선 | v1.24.0 | 🟡 보류 (점진적 개선) |

### 해소 완료

| ID | Check | 심각도 | 유형 | 설명 | 발견 버전 | 해소 버전 | 상태 |
|----|-------|--------|------|------|----------|----------|------|
| DV-NEW-066 | 1.2 | Low | 증상 | 고아 test Chunk 10건 재발 삭제 (test, study, paper, to_delete, integration_test, paper_0~2, temp_doc) | v1.30.0 | v1.30.0 | ✅ 10건 삭제 |
| DV-NEW-067 | 1.3/5.6 | Low | 증상 | 대소문자 중복 엔티티 15쌍 병합 (I:11, O:3, A:1) → APOC mergeNodes | v1.30.0 | v1.30.0 | ✅ 15쌍 병합 |
| DV-NEW-068 | 5.2 | Low | 증상 | study_design 정규화 14건 (other→Other 13건, meta-analysis→Meta-Analysis 1건) | v1.30.0 | v1.30.0 | ✅ 14건 정규화 |
| DV-NEW-069 | 2.3 | Medium | 증상 | TREATS 누락 133건 백필 (INVESTIGATES+STUDIES 기반 paper_count 포함) | v1.30.0 | v1.30.0 | ✅ 133건 생성 |
| DV-NEW-062 | 5.3 | High | 증상 | DOI 중복 Paper 6쌍 (pubmed_ vs doi_ 재발) → doi_ 논문 6건 삭제 | v1.25.0 | v1.25.0 | ✅ 6건 삭제 |
| DV-NEW-061 | 1.1/5.4 | Medium | 증상 | Chunk paper_id 누락 93건 HAS_CHUNK 기반 복구 + 미존재 paper_id Chunk 10건 삭제 | v1.25.0 | v1.25.0 | ✅ 93건 복구, 10건 삭제 |
| DV-NEW-060 | 2.1 | Medium | 증상 | 고립 Paper 11건 → doi_ 6건 삭제, LLM 복구 3건, 제목기반 2건(abstract 미등록 ahead-of-print) | v1.25.0 | v1.25.0 | ✅ 0건 잔여 |
| DV-NEW-059 | 1.3/5.6 | Low | 증상 | 대소문자 중복 엔티티 16건 (I:9, P:2, O:3, A:2) → 관계 이전 후 병합 | v1.25.0 | v1.25.0 | ✅ 16건 병합 |
| DV-NEW-058 | 5.2 | Low | 증상 | study_design 'other' 26건 → 'Other' 정규화 | v1.25.0 | v1.25.0 | ✅ 26건 정규화 |
| DV-NEW-057 | 2.3 | Low | 증상 | TREATS 미생성 74건 백필 + paper_count 3219건 갱신 | v1.25.0 | v1.25.0 | ✅ 74건 백필, 3219건 갱신 |
| DV-NEW-056 | 1.2 | Low | 증상 | 고아 test Chunk 10건 삭제 (paper_2, temp_doc, paper, test, study 등) | v1.25.0 | v1.25.0 | ✅ 10건 삭제 |
| DV-NEW-055 | 2.3/6.6 | Low | 증상 | TREATS 미연결 Intervention 7개 → 67건 TREATS 백필 (co-study + sibling 기반) | v1.25.0 | v1.25.0 | ✅ 67건 백필 |
| DV-NEW-054 | 5.2 | Low | 증상 | study_design 대소문자/kebab-case 비정규 48건 → canonical 값으로 정규화 | v1.25.0 | v1.25.0 | ✅ 48건 정규화 |
| DV-NEW-053 | 5.4 | Low | 증상 | Chunk integer tier 228건 (1→'tier1') 변환 | v1.25.0 | v1.25.0 | ✅ 228건 변환 |
| DV-NEW-052 | 5.3 | Medium | 증상 | DOI 중복 Paper 4쌍 (pubmed_ vs doi_) → 관계 이전 후 중복 삭제 | v1.25.0 | v1.25.0 | ✅ 4건 병합 |
| DV-NEW-051 | 4.5 | Low | 정보 | Neurogenic claudication SNOMED 코드 소스 일치 확인 | v1.25.0 | v1.25.0 | ✅ 확인 |
| DV-NEW-050 | 2.3 | Low | 증상 | TREATS paper_count 불일치 2808건 갱신 | v1.25.0 | v1.25.0 | ✅ 2808건 갱신 |
| DV-NEW-049 | 6.2/2.2 | Medium | 원인 | IS_A 순환: Neuromodulation↔SCS → 잘못된 방향 삭제 | v1.25.0 | v1.25.0 | ✅ 1건 삭제 |
| DV-NEW-048 | 1.3 | Medium | 증상 | 대소문자 중복 엔티티 15쌍 (I:5, P:9, A:1) → 관계 이전 후 병합 | v1.25.0 | v1.25.0 | ✅ 15쌍 병합 |
| DV-NEW-047 | 1.2 | Low | 증상 | 고아 test Chunk 5건 삭제 | v1.25.0 | v1.25.0 | ✅ 5건 삭제 |
| DV-NEW-041 | 2.1 | Medium | 증상 | 고립 Paper 10건 → abstract 확보(PubMed 2건, arXiv 2건, OpenAlex/제목기반 2건) + LLM 복구 10건 전체 해소 | v1.25.0 | v1.25.0 | ✅ 10건 복구 |
| DV-NEW-039 | 1.1 | Medium | 증상 | Chunk paper_id 누락 214건 → HAS_CHUNK 역추적 백필 | v1.25.0 | v1.25.0 | ✅ 214건 백필 |
| DV-NEW-040 | 1.2 | Low | 증상 | 고아 test Chunk 10건 재발 → 삭제 | v1.25.0 | v1.25.0 | ✅ 10건 삭제 |
| DV-NEW-042 | 6.2/2.2 | Medium | 원인 | IS_A 순환 4건: Neuromodulation↔SCS, PROMs↔Patient-Reported Outcome → 잘못된 방향 삭제 | v1.25.0 | v1.25.0 | ✅ 2건 삭제 |
| DV-NEW-043 | 5.3 | Low | 증상 | 중복 제목 Paper 6→2건 (text_ 중복 4건 삭제, Unknown 2건+이중출판 1건 보류) | v1.25.0 | v1.25.0 | ✅ 4건 삭제 |
| DV-NEW-044 | 5.4 | Medium | 증상 | 비정규 tier 170건 (integer 1/2 → string tier1/tier2) | v1.25.0 | v1.25.0 | ✅ 170건 변환 |
| DV-NEW-045 | 4.5 | Medium | 증상 | SNOMED 소스↔Neo4j 불일치 43건 (I:2, P:15, O:26) → 소스 기준 업데이트 | v1.25.0 | v1.25.0 | ✅ 43건 수정 |
| DV-NEW-046 | 1.3/5.6 | Low | 증상 | Intervention 대소문자 중복 1건 (AI-based Cobb angle/Angle) → 병합 | v1.25.0 | v1.25.0 | ✅ 1건 병합 |
| DV-NEW-026 | 6.5 | Medium | 원인 | 순환 IS_A 3쌍 제거 (Neuromodulation↔SCS, PROMs↔Patient-Reported Outcome, Quality of Life Outcome↔EQ-5D) | v1.25.0 | v1.25.0 | ✅ 잘못된 방향 삭제 |
| DV-NEW-027 | 1.2/5.4 | Low | 증상 | 고아 test Chunk 10건 삭제 (test, study, paper, to_delete, integration_test 등) | v1.25.0 | v1.25.0 | ✅ 10건 삭제 |
| DV-NEW-028 | 6.3 | Low | 증상 | 고아 Pathology 3건 IS_A 연결 (Spinal curvature, Spinal Alignment Abnormalities, Lordosis → Spinal Deformity) | v1.25.0 | v1.25.0 | ✅ IS_A 연결 |
| DV-NEW-029 | 6.3 | Low | 증상 | 고아 AI Intervention 7건 IS_A 연결 (AI-guided measurement 등 → Automated Radiograph Analysis) | v1.25.0 | v1.25.0 | ✅ IS_A 연결 |
| DV-NEW-030 | 2.3 | Medium | 증상 | TREATS 누락 26건 백필 (INVESTIGATES+STUDIES 기반) | v1.25.0 | v1.25.0 | ✅ 26건 생성 |
| DV-NEW-031 | 2.3 | Low | 증상 | TREATS paper_count 불일치 681건 갱신 | v1.25.0 | v1.25.0 | ✅ 681건 갱신 |
| DV-NEW-032 | 6.8 | Medium | 증상 | SNOMED 코드 충돌 해소: Lumbar arthrodesis (341→132), Tokuhashi Score (341 유지) | v1.25.0 | v1.25.0 | ✅ 코드 수정 |
| DV-NEW-033 | 6.8 | Info | 정보 | Pathology-Outcome cross-label name overlap 20쌍 — Known Accepted (DV-A-011)로 이동 | v1.25.0 | v1.25.0 | ✅ DV-A-011 허용 등록 |
| DV-NEW-006 | 4.2/6.4 | Info | 정보 | SNOMED 커버리지 — Known Accepted (DV-A-008)로 이동 | v1.23.4 | v1.24.1 | ✅ DV-A-008 허용 등록 |
| DV-NEW-009 | 5.3 | Info | 정보 | 동일 제목 Paper 1쌍 — Known Accepted (DV-A-009)로 이동 | v1.23.4 | v1.24.1 | ✅ DV-A-009 허용 등록 |
| DV-NEW-011 | 5.2 | Low | 원인 | study_design 정규화 — normalize_study_design() 추가, 471건 마이그레이션, 11개 canonical 값, 파이프라인 6곳 적용 | v1.23.4 | v1.24.1 | ✅ 코드+데이터 해소 |
| DV-NEW-013 | 4.5 | Medium | 원인 | SNOMED source↔Neo4j 불일치 — enrich 재실행 해소, 잔여는 DV-A-006 허용 | v1.24.0 | v1.24.1 | ✅ 허용 처리 |
| DV-NEW-016 | 6.5 | Medium | 원인 | Intervention parent_code↔IS_A — schema.py에 Intervention taxonomy 추가 + build/repair_ontology 실행, 39.2%→100% 일치 | v1.24.0 | v1.24.1 | ✅ 코드+데이터 해소 |
| DV-NEW-017 | 2.3 | Low | 증상 | TREATS paper_count 불일치 — enrich 재실행으로 재계산 해소 | v1.24.0 | v1.24.1 | ✅ 해소 |
| DV-NEW-018 | 3.2 | Low | 증상 | Abstract 임베딩 누락 — 재스캔 해소 확인 | v1.24.0 | v1.24.1 | ✅ 해소 |
| DV-NEW-019 | 6.3 | Low | 정보 | 고아 리프 노드 — 3건 진정한 고립 노드 삭제, 나머지는 IS_A로 연결됨 | v1.24.0 | v1.24.1 | ✅ 삭제+정리 |
| DV-NEW-020 | 6.6 | Low | 증상 | Robot-assisted fusion AFFECTS 미연결 — Known Accepted (DV-A-010)로 이동 | v1.24.0 | v1.24.1 | ✅ DV-A-010 허용 등록 |
| DV-NEW-021 | 1.2/5.4 | Low | 증상 | 고아 test Chunk — 9건 삭제 | v1.24.0 | v1.24.1 | ✅ 삭제 |
| DV-NEW-022 | 5.4 | Medium | 증상 | Chunk integer tier — 재스캔 시 0건 확인 (이미 변환 완료) | v1.24.0 | v1.24.1 | ✅ 해소 |
| DV-NEW-024 | 4.5 | Medium | 증상 | Cross-label SNOMED 중복(Lumbar arthrodesis vs Tokuhashi Score, code 341) | v1.24.1 | v1.24.1 | ✅ enrich_graph_snomed.py 재실행으로 해소 |
| DV-NEW-025 | 4.5 | Low | 증상 | SNOMED source↔Neo4j mismatch 매핑 확장 후 증가 | v1.24.1 | v1.24.1 | ✅ enrich_graph_snomed.py 재실행으로 해소 |
| DV-NEW-012 | 1.1/5.4 | Medium | 증상 | Chunk paper_id NULL 240건 — v1.24.1 재스캔에서 해소 확인 | v1.24.0 | v1.24.1 | ✅ 해소 |
| DV-NEW-014 | 1.3/4.4 | Medium | 증상 | 대소문자 중복 노드 — v1.24.1 재스캔에서 해소 확인 | v1.24.0 | v1.24.1 | ✅ 해소 |
...
|------|------|----------|------|----------|--------------|------|
| 2026-03-18 | v1.30.0 | 4 | 4 | 1 | 11 | (2차) DV-NEW-066~069: 고아chunk 10건 삭제, 대소문자중복 15쌍 병합(I:11,O:3,A:1), study_design 14건 정규화(other 13+meta-analysis 1), TREATS 133건 백필. DB: Paper 773, TREATS 5276 |
| 2026-03-18 | v1.30.0 | 4 | 4 | 1 | 11 | 고아chunk 10건 삭제, 대소문자중복 19건 병합(I:14,P:4,A:2→0), TREATS 618건 갱신, study_design 30건 정규화(other 27+meta-analysis 2+Cross-sectional 1). DB: Paper 773, Chunk 11774 |
| 2026-03-18 | v1.29.0 | 7 | 7 | 1 | 11 | (3차) 고아chunk 10건 삭제, 대소문자중복 8건 병합(I:3 ACR/IF/PF, O:5 HRQoL/IB/LK/SI/WA), TREATS 980건 갱신(34 mismatch+946 missing), study_design 1건 추가(pubmed_41673318→Cross-sectional study). DB: Paper 743, Chunk 11656 |
| 2026-03-18 | v1.29.0 | 6 | 6 | 1 | 11 | (2차) 고아chunk 20건 삭제, 대소문자중복 4건 병합(Anterior Fixation/Fusion, Neurogenic Claudication, cervical), 고립Paper 4건 복구(cervical trauma), TREATS 166건 갱신, study_design 90건 정규화(other→Other, meta-analysis→Meta-Analysis 등). EL 정합성 양호(7개 정규값, 비정규 0). DB: Paper 743, Chunk 10678 |
| 2026-03-17 | v1.27.0 | 8 | 7 | 1 | 11 | DV-NEW-001~008: 고아chunk 10건 삭제, 대소문자중복 3건 병합, TREATS paper_count 270건 갱신, study_design 12건 정규화, pmid_34559764 EL unknown→5. DV-NEW-004(Paper임베딩 418건 미존재) info→보류, DV-NEW-008(Other study_design 172건) info. EL 정합성 양호(638편 중 1편만 비정규). DB: Paper 638, Chunk 9869 |
| 2026-03-16 | v1.25.0 | 7 | 7 | 1 | 11 | DV-NEW-056~062 발견+즉시해소: 056(고아Chunk 10건 삭제), 057(TREATS 74건 백필+3219건 갱신), 058(study_design 26건 정규화), 059(대소문자중복 16건 병합), 060(고립Paper 11건→doi_ 삭제6+LLM복구3+제목기반2), 061(Chunk paper_id 93건 복구+10건 삭제), 062(DOI중복 6쌍 삭제). DB: Paper 618, Chunk 9491 |
| 2026-03-16 | v1.25.0 | 9 | 9 | 1 | 11 | QA 전체 재스캔 (QC/CA/DV). DV-NEW-047~055 발견+즉시해소: 047(고아Chunk 5건 삭제), 048(대소문자중복 15쌍 병합), 049(IS_A순환 Neuromodulation↔SCS 해소), 050(TREATS paper_count 2808건 갱신), 051(Neurogenic claudication SNOMED 확인), 052(DOI중복Paper 4건 병합), 053(tier정수→문자열 228건), 054(study_design 정규화 48건), 055(TREATS 미연결 7개 Intervention→67건 백필). SNOMED 39개 추가(696→735, I:235,P:231,O:200,A:69). entity_normalizer 분리(4888→1366줄+normalization_maps 3543줄). DB: Paper 527, Chunk 9022 |
| 2026-03-16 | v1.25.0 | 8 | 8 | 1 | 11 | DV-NEW-039~046 발견+해소. 039(Chunk paper_id 214건 백필), 040(고아Chunk 10건 삭제), 041(고립Paper 10건→abstract확보+LLM복구 전체해소), 042(IS_A순환 4건 해소), 043(중복제목 6→1건, text_4건삭제+Unknown제목수정), 044(비정규tier 170건 변환), 045(SNOMED불일치 43건 소스기준 업데이트), 046(Intervention중복 1건 병합). DV-NEW-015: build/repair_ontology 실행→I:57%,P:51%,O:90%,A:36%. 🤖 신규DV항목: 4.7/5.5/5.6 LLM체크 추가. DB: Paper 531, Chunk 9017, Rels 33321 |
| 2026-03-13 | v1.25.0 | 5 | 2 | 4 | 11 | DV-NEW-034(MENTIONS/APPLIED_TO dataclass 미정의, Low), 035(Schema 정합성 PASS, Info), 036(Intervention parent_code 69건 누락, Low), 037(EvidenceLevel enum 7→9값 동기화, Medium→즉시 수정), 038(InterventionCategory +navigation, Low→즉시 수정). DV-039(미사용 enum, Info) |
| 2026-03-02 | v1.25.0 | 8 | 8 | 1 | 11 | DV-NEW-026~033 발견+해소: 순환 IS_A 3쌍 해소, 고아 Chunk 10건 삭제, 고아 Pathology 3건/Intervention 7건 IS_A 연결, TREATS 백필 26건+갱신 681건, SNOMED 충돌 해소(341→132), cross-label overlap→DV-A-011 허용. 잔여 1건: DV-NEW-015 |
| 2026-02-28 | v1.24.1 | 0 | 11 | 1 | 10 | 전면 해소 스프린트: DV-NEW-006/009/020→Known Accepted(A-008/A-009/A-010), DV-NEW-011(study_design 정규화 코드+데이터), DV-NEW-016(IS_A 100% 일치), DV-NEW-019(고아 삭제), DV-NEW-021/022(테스트 잔여물 정리). 잔여 1건: DV-NEW-015(IS_A 커버리지 모니터링) |
| 2026-02-28 | v1.24.1 | 3 | 6 | 9 | 7 | enrich_graph_snomed.py 재실행 후 스캔. DV-NEW-023(IS_A root→A-007 허용), 024(code 341 중복→해소), 025(source↔Neo4j mismatch→해소). DV-NEW-012/014/017/018 해소. 커버리지: I:44.2%, P:57.2%, O:26.1%, A:91.1% |
