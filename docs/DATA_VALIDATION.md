...
|----|-------|--------|------|------|----------|------|
| DV-NEW-006 | 4.2/6.4 | Info | 정보 | SNOMED 커버리지 낮음 — v1.24.0 기준: I:38.1%, P:32.2%, O:21.7% (A:86.5% OK). v1.24.x에서 매핑 621→653개 확장 + ~280 alias 추가 완료. `enrich_graph_snomed.py` 재실행 시 개선 예상 | v1.23.4 | 🟡 허용 (enrich 실행 대기) |
...
| DV-NEW-013 | 4.5 | Medium | 원인 | SNOMED source↔Neo4j 코드 불일치 다수 — v1.24.0 기준 I:1건, P:5건, O:6건. v1.24.x에서 매핑 621→653 확장 + PCO→SPO 통합 등 코드 변경으로 추가 불일치 예상. `enrich_graph_snomed.py` 재실행으로 해소 | v1.24.0 | 🔴 신규 (enrich 실행 대기) |
