# Spine GraphRAG v1.22.0 - Deployment Guide
...
tar --exclude='.venv' --exclude='__pycache__' --exclude='.git' \
    --exclude='logs' \
    -czvf rag_research_v1.22.0.tar.gz rag_research/

scp rag_research_v1.22.0.tar.gz user@newserver:~/

# 새 서버에서 압축 해제
ssh user@newserver "cd ~ && tar -xzvf rag_research_v1.22.0.tar.gz"
```
...
echo "=== Spine GraphRAG v1.22.0 Deployment Verification ==="
