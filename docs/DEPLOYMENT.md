# Spine GraphRAG v1.23.1 - Deployment Guide
...
| **Version** | 1.23.1 |
| **Date** | 2026-02-17 |
| **SNOMED Mappings** | 447개 (I:168, P:125, O:108, A:46) + 패턴 매핑 |
...
- **SNOMED-CT 매핑**: 447개 매핑 (I:168, P:125, O:108, A:46)
...
tar --exclude='.venv' --exclude='__pycache__' --exclude='.git' \
    --exclude='logs' \
    -czvf rag_research_v1.23.1.tar.gz rag_research/

scp rag_research_v1.23.1.tar.gz user@newserver:~/

# 새 서버에서 압축 해제
ssh user@newserver "cd ~ && tar -xzvf rag_research_v1.23.1.tar.gz"
```
...
echo "=== Spine GraphRAG v1.23.1 Deployment Verification ==="
