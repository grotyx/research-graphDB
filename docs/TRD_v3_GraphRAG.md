...
| **Vector Storage** | ChromaDB | ChromaDB (유지) |
...
```
┌─────────────────────────────────────────────────────────────────────┐
│         Claude Haiku 4.5 PDF Processor (v7.14+)                      │
...
└─────────────────────────────────────────────────────────────────────┘
                ...
                ▼                               ▼
┌─────────────────────────────┐   ┌─────────────────────────────────┐
│   Neo4j Graph DB (신규)      │   │   ChromaDB Vector DB (유지)      │
├─────────────────────────────┤   ├─────────────────────────────────┤
│ ▸ 구조적 지식 저장:          │   │ ▸ 문맥 저장:                     │
│   - Paper nodes             │   │   - Tier1/2 청크                 │
│   - Pathology nodes         │   │   - Discussion, Introduction    │
│   - Anatomy nodes           │   │   - 의미 검색용 임베딩           │
│   - Intervention nodes      │   │                                  │
│   - Outcome nodes           │   │ ▸ 텍스트 의미 검색               │
│                              │   │                                  │
│ ▸ 관계 (Edges):              │   └─────────────────────────────────┘
│   - (Paper)-[STUDIES]->     │
│   - (Intervention)-[TREATS]->│
...
│   ├── doi_fulltext_fetcher.py       # ★ v7.13: DOI/Unpaywall 전문 조회
...
│
├── storage/                          # 저장소
│   ├── vector_db.py                  # ChromaDB (유지)
│   └── graph_db.py                   # ★ 신규: Neo4j 래퍼
│
...
└── medical_mcp/                      # MCP 서버
    ...
    └── handlers/                     # ★ v7.14: 11개 도메인별 핸들러 + utils
        ...
        ├── reference_handler.py      # ★ v7.9: 참고문헌 포맷팅 (7가지 스타일)
        ├── writing_guide_handler.py  # ★ v7.12: 학술 논문 작성 가이드 (9개 체크리스트)
        └── utils.py                  # 공유 유틸리티
...
└─────────────────────────────────────────────────────────────────────┘
    ...
    ▼                   ▼                      ▼                     ▼
┌─────────────┐  ┌─────────────────┐  ┌────────────────┐  ┌──────────────┐
│ ChromaDB    │  │ Neo4j Graph     │  │ Entity         │  │ Relationship │
│ (Vector)    │  │ (Structure)     │  │ Normalizer     │  │ Builder      │
├─────────────┤  ├─────────────────┤  ├────────────────┤  ├──────────────┤
│ Tier1/2 청크 │  │ ★ 자동 생성:     │  │ 용어 통일:      │  │ ★ 신규 통합:  │
│ 임베딩 저장   │  │                 │  │ UBE ↔ BESS    │  │ PDF 업로드 시 │
│ 메타데이터    │  │ 1. Paper 노드   │  │ Biportal      │  │ Neo4j에 자동  │
...
├─────────────────┤ ├─────────────────────┤ ├──────────────────────┤
│ Cypher:         │ │ ChromaDB Tier1:     │ │ ChromaDB Tier2:      │
│ MATCH           │ │ - "ASD" 관련 청크    │ │ - Background 정보     │
│ (i:Intervention)│ │ - "PJK" 관련 청크    │ │ - Discussion 섹션     │
│ -[a:AFFECTS]->  │ │ - Semantic 유사도    │ │                      │
│ (o:Outcome      │ │   기반 검색          │ │                      │
│  {name:"PJK"})  │ │                     │ │                      │
│ WHERE           │ │                     │ │                      │
│ a.direction =   │ │                     │ │                      │
...
async def add_pdf(pdf_path: str):
    # 1. Gemini Vision 처리
    result = await vision_processor.process_pdf(pdf_path)

    # 2. ChromaDB에 벡터 저장 (기존)
    await vector_db.add_chunks(result.chunks)

    # 3. ★ Neo4j에 그래프 관계 자동 구축 (신규)
    if GRAPH_AVAILABLE and neo4j_client:
        # SpineMetadata 추론
...
```
src/llm/gemini_client.py           # API 클라이언트 유지
src/llm/cache.py                   # 캐싱 유지
src/storage/vector_db.py           # ChromaDB 유지
src/external/pubmed_client.py      # PubMed 유지
src/builder/gemini_vision_processor.py  # Vision 처리 유지 (스키마 확장)
```
...
### 8.1 requirements.txt 추가

```
# v3.0 GraphRAG 신규 의존성

# Neo4j
neo4j>=5.15.0                     # Neo4j Python Driver
neomodel>=5.2.0                   # OGM (선택적)

# LangChain
langchain>=0.1.0                  # Orchestration
langchain-community>=0.0.10       # Neo4j integration
...
class SpineRAGAgent:
    ...
    def __init__(self):
        self.tools = [
            GraphSearchTool(self.neo4j),
            VectorSearchTool(self.chromadb),
