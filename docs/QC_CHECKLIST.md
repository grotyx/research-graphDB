...
불일치하는 항목이 있으면 src/__init__.py 기준으로 수정해줘.
```
...
| OneDrive 가이드 | OneDrive CLAUDE.MD (외부) |

**Claude Code 프롬프트:**
```
src/medical_mcp/medical_kag_server.py에서 name="..." 패턴으로 등록된
MCP 도구 이름 목록을 추출하고, 아래 문서들의 도구 이름과 비교해줘:
- docs/user_guide.md
...
실제 코드 수와 문서 수가 다르면 문서를 코드 기준으로 수정해줘.
```
...
PYTHONPATH=./src ./.venv/bin/python -m pytest tests/ -v --tb=short 2>&1 | tail -50
```

**기대 결과:** 모든 테스트 PASSED (현재 139+ 테스트)
...
# 또는 도구 목록 확인
curl -s http://localhost:7777/tools | python3 -m json.tool
```
...
```
Phase 1 (병렬)          Phase 2 (병렬)          Phase 3 (순차)       Phase 4 (병렬)
├─ 1.1 버전 동기화      ├─ 2.1 테스트 실행      ├─ 3.1 Docker       ├─ 4.1 CHANGELOG
├─ 1.2 문서 참조        ├─ 2.2 Import 체크      ├─ 3.2 Neo4j        ├─ 4.2 스키마 문서
├─ 1.3 MCP 도구 이름    ├─ 2.3 안티패턴         ├─ 3.3 API 키       └─ 4.3 Taxonomy 문서
